...
public class MainTabbedPanel<T extends JComponent & UriGettable> extends TabbedPanel<T> implements UriOpenable, PreferencesChangeListener, PageChangeListener {
    ...
    public void create() {
        ...
        JLabel title = newLabel("아무것도 열리지 않았습니다.", fontColor);
        title.setFont(title.getFont().deriveFont(Font.BOLD, title.getFont().getSize()+8));

        box.add(title);
        box.add(newLabel("\"파일 > 파일 열기...\"에서 파일을 여세요.", fontColor));
        box.add(newLabel("\"파일 > 최근 파일 목록\"에서 최근 파일을 여세요.", fontColor));
        box.add(newLabel(getFileManagerLabel() + "에서 파일을 드래그 & 드롭해 여세요.", fontColor));
        box.add(Box.createVerticalGlue());
